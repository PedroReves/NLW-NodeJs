...
export default async function createInvite(app: FastifyInstance) {
	app.withTypeProvider<ZodTypeProvider>().post(
		...
		async (request) => {
			...
			const formattedStartDate = dayjs(trip.starts_at).format("LL"); // DD -> Dias, MMMM
			const formattedEndDate = dayjs(trip.ends_at).format("LL"); // DD -> Dias, MMMM
            ...
            <div style="font-family: sans-serif; font-size: 16px; line-height: 1.6;">
              <p>Você solicitou uma viagem para <strong>${trip.destination}</strong> nas datas de <strong>${formattedStartDate}</strong> até <strong>${formattedEndDate}</strong></p>
              <p></p>
